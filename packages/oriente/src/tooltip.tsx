...
export interface TooltipProps extends StyleProps<[TooltipProps]> {
  ...
  children: React.ReactElement<any> | ((props: TooltipChildrenProps) => React.ReactNode);
...
export interface TooltipArrowProps
  extends StyleProps<[TooltipArrowProps]>, React.HTMLProps<HTMLDivElement> {
...
const Tooltip = (inProps: TooltipProps) => {
  ...
  useEffect(() => {
    ...
    const openByHover = showOnHover && (tapState.isHovered || tooltipTaply.tapState.isHovered);
    ...
    (ref: React.Ref<HTMLDivElement>) =>
      tooltipTaply.render((attrs, taplyRef) => (
        <div ref={mergeRefs(ref, taplyRef)} {...attrs}>
          ...
          >
            <TooltipContext.Provider value={context}>{tooltip}</TooltipContext.Provider>
          </OpenAnimation>
        </div>
      )),
  ...
  return (
    <Popup isActive={isOpen || !isRest} placement={placement} popup={popup} onChangeSide={setSide}>
      {target}
    </Popup>
  );
};
